...
class DialogTableViewCellModel: NSObject {
    ...
    init(dialog: QBChatDialog) {
		...
		case .private:
            ...
            if let recipient = QMServicesManager.instance().usersService.usersMemoryStorage.user(withID: UInt(dialog.recipientID)) {
...
//     let loginInfoDictionary = NSKeyedUnarchiver.unarchiveObject(with: kUserDefault.value(forKey: kloginInfo) as! Data) as! NSMutableDictionary
        ...
        let qbID : Int = Int(chatDialog.userID)//loginInfoDictionary[kqbId] as! Int
        let imageUrl = "http://api.frinck.com/v1/user/chatprofileimage/\(qbID)"
        let url : URL = URL(string: imageUrl)!
        cell.dialogTypeImage.sd_setImage(with: url, completed:nil)
        return cell
    }
